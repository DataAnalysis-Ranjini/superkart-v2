...
with tab1:
    ...
    # 1. Perishable / Non-Perishable selection comes first
    perishable_category = st.selectbox(
        "Product Category Type", 
        ["Perishable Food", "Non-Perishable Food", "Non-Edible / General Merchandise"]
    )
    
    # 2. Dynamically filter Product Types based on the category selection
    if perishable_category == "Perishable Food":
        product_type = st.selectbox("Product Type", ["Dairy", "Meat", "Fruits and Vegetables", "Seafood", "Breads", "Frozen Foods"])
        sugar_content = st.selectbox("Product Fat Content", ["Low Fat", "Regular"])
        category_code = "FD"
    elif perishable_category == "Non-Perishable Food":
        product_type = st.selectbox("Product Type", ["Soft Drinks", "Baking Goods", "Canned", "Starchy Foods", "Hard Drinks", "Others"])
        sugar_content = st.selectbox("Product Fat Content", ["Low Fat", "Regular"])
        category_code = "FD"
    else:
        product_type = st.selectbox("Product Type", ["Household", "Health and Hygiene"])
        sugar_content = "Non-Edible"
        category_code = "NC"
